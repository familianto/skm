...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Count active transactions using this kategori
    const count = rows.filter((row) => {
      return row[kategoriIdx] === id && row[statusIdx] !== TransaksiStatus.VOID;
    }).length;
    ...
    console.error('GET /api/kategori/[id]/usage-count error:', error);
    return NextResponse.json<ApiResponse<null>>(
      { success: false, error: 'Gagal menghitung penggunaan kategori.' },
      { status: 500 }
    );
  }
}
