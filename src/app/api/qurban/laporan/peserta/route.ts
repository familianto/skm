import { NextRequest } from 'next/server';

import { success, error } from '@/lib/api/response';
import { ErrorCodes } from '@/lib/api/errors';
import { requireRole } from '@/lib/api/guards';
import { PERAN } from '@/lib/api/permissions';

import { findActiveEdisi, findEdisiById } from '@/lib/qurban/edisi-repo';
import { evaluatePesertaEdisiGate } from '@/lib/qurban/peserta-context';
import { listPesertaByEdisi } from '@/lib/qurban/peserta-repo';
import { listPembayaranByEdisi } from '@/lib/qurban/pembayaran-repo';
import { listDaftarHewanByEdisi } from '@/lib/qurban/daftar-hewan-repo';
import { listAllMuqorib } from '@/lib/qurban/muqorib-repo';
import { isEdisiArsip } from '@/lib/qurban/laporan-dashboard';
import { buildLaporanPeserta } from '@/lib/qurban/laporan-peserta';

const READ_ROLES = [
  PERAN.SUPER_ADMIN,
  PERAN.BENDAHARA,
  PERAN.ADMIN_QURBAN,
  PERAN.PENDAFTARAN,
  PERAN.DISTRIBUSI,
];

/**
 * LP1 — GET /api/qurban/laporan/peserta?edisi_id=EDS-...
 *
 * Agregasi read-only Laporan Peserta (F8 Milestone B). Semua role login boleh.
 * `edisi_id` opsional → default edisi AKTIF. Panitia (PENDAFTARAN/DISTRIBUSI)
 * hanya boleh edisi AKTIF (gate `peserta-context`, reuse Milestone A).
 *
 * Mengembalikan KETIGA grouping (tipe / jenis_kelas / rt) dalam satu respons —
 * UI ganti grouping instan tanpa refetch. TIDAK menulis apa pun & TIDAK
 * memanggil withAuditLog. Baca tiap sheet sekali, join in-memory.
 */
export async function GET(request: NextRequest) {
  const guard = await requireRole(request, READ_ROLES);
  if (!guard.ok) return guard.response;

  try {
    const url = new URL(request.url);
    const edisiId = (url.searchParams.get('edisi_id') || '').trim();

    const edisi = edisiId ? await findEdisiById(edisiId) : await findActiveEdisi();
    const decision = evaluatePesertaEdisiGate(edisi, guard.session.peran, {});
    if (!decision.ok) {
      return error(decision.code, decision.message, decision.status, decision.details);
    }
    const resolved = edisi!;

    const [peserta, hewan, muqorib, pembayaran] = await Promise.all([
      listPesertaByEdisi(resolved.id),
      listDaftarHewanByEdisi(resolved.id),
      listAllMuqorib(),
      listPembayaranByEdisi(resolved.id),
    ]);

    const today = new Date().toISOString().slice(0, 10);
    const dto = buildLaporanPeserta({
      edisi: resolved,
      isArsip: isEdisiArsip(resolved, pembayaran, today),
      peserta,
      hewan,
      muqorib,
    });

    return success(dto, { generated_at: new Date().toISOString() });
  } catch (err) {
    console.error('[GET /api/qurban/laporan/peserta] error:', err);
    return error(ErrorCodes.INTERNAL_ERROR, 'Gagal memuat laporan peserta.', 500);
  }
}
